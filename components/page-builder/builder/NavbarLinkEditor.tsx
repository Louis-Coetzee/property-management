...
import { X, ChevronDown, ExternalLink, FileText, Hash, MessageSquare, Image as ImageIcon, Sparkles, Plus, ChevronRight, GripVertical } from 'lucide-react';
import { cn } from '@/lib/utils';
import type { NavbarLink, NavbarSubLink, PageSection } from '@/types/page-builder';
import { useQuery } from 'convex/react';
import { api } from '@/convex/_generated/api';
import MediaLibraryModal from '@/components/media-library-modal';

interface NavbarLinkEditorProps {
  link: NavbarLink;
  index: number;
  websiteId: string;
  userId?: string;
  onUpdate: (index: number, link: NavbarLink) => void;
  onRemove: (index: number) => void;
}

// Sublink Editor Component
function SublinkEditor({
  ...
  sublink: NavbarSubLink;
  subIndex: number;
  websiteId: string;
  userId?: string;
  onUpdate: (subIndex: number, sublink: NavbarSubLink) => void;
  ...
  const selectedPage = pages?.find(p => p._id === sublink.pageId);
  const pageContent = selectedPage?.content ? JSON.parse(selectedPage.content) : null;
  const sections = pageContent?.sections || [];

  const linkType = sublink.type || 'url';
  ...
  const handleTypeChange = (type: 'url' | 'page' | 'form') => {
    onUpdate(subIndex, {
      ...sublink,
      type,
      url: type === 'url' ? (sublink.url || '') : undefined,
      pageId: type === 'page' ? (sublink.pageId || undefined) : undefined,
      sectionId: type === 'page' ? (sublink.sectionId || undefined) : undefined,
      formId: type === 'form' ? (sublink.formId || undefined) : undefined,
    });
  ...
  const handleSectionSelect = (sectionId: string) => {
    onUpdate(subIndex, { ...sublink, sectionId });
    setShowSectionDropdown(false);
  };

  const handleFormSelect = (formId: string) => {
    onUpdate(subIndex, { ...sublink, formId });
    setShowFormDropdown(false);
  };

  const getSelectedPageName = () => {
    if (!sublink.pageId) return 'Select a page';
    const page = pages?.find(p => p._id === sublink.pageId);
    return page?.name || 'Select a page';
  };

  const getSelectedSectionName = () => {
    if (!sublink.sectionId) return 'All sections';
    const section = sections.find((s: PageSection) => s.id === sublink.sectionId);
  ...
  const handleMediaLibrarySelect = (urls: string[]) => {
    if (urls.length > 0) {
      if (mediaType === 'main') {
        onUpdate(subIndex, { ...sublink, imageUrl: urls[0] });
      } else {
        onUpdate(subIndex, { ...sublink, hoverImageUrl: urls[0] });
      }
    }
  ...
  return (
    <div className="border border-slate-200 rounded-lg">
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2.5 bg-slate-50 hover:bg-slate-100 transition-colors rounded-lg"
      >
        <div className="flex items-center gap-2">
          <GripVertical className="h-3.5 w-3.5 text-slate-400 cursor-grab" />
          <span className="text-sm font-medium text-slate-900">
            {sublink.label || `Sublink ${subIndex + 1}`}
          </span>
          {isUrlType ? (
      ...
      {/* Expanded Content */}
      {isOpen && (
        <div className="p-3 space-y-3 bg-white border-t border-slate-200 rounded-b-lg">
          {/* Label */}
          <div>
            ...
            <input
              type="text"
              value={sublink.label}
              onChange={(e) => onUpdate(subIndex, { ...sublink, label: e.target.value })}
              placeholder="Menu Item"
              className="w-full px-2.5 py-1.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onUpdate(subIndex, { ...sublink, description: e.target.value })}
          ...
          {isUrlType && (
            <div>
              ...
              <input
                type="text"
                value={sublink.url || ''}
                onChange={(e) => onUpdate(subIndex, { ...sublink, url: e.target.value })}
                placeholder="https://example.com or /about"
                className="w-full px-2.5 py-1.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent font-mono"
              />
            </div>
          )}
            ...
            <>
              <div className="relative">
                ...
                <button
                  ...
                  className="w-full px-2.5 py-1.5 text-sm text-left bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 flex items-center justify-between"
                >
                  <span className={cn(!sublink.pageId && 'text-slate-400')}>
                    {getSelectedPageName()}
                  </span>
                      ...
                      pages.map((page) => (
                        <button
                          ...
                          className={cn(
                            ...
                            sublink.pageId === page._id && 'bg-slate-100 font-medium'
                          )}
                        >
                          {page.name}
                          {page.isHomePage && (
              ...
              {/* Section Selection */}
              {sublink.pageId && sections.length > 0 && (
                <div className="relative">
                  <label className="block text-xs font-medium text-slate-700 mb-1">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-2.5 py-1.5 text-sm text-left bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 flex items-center justify-between"
                    >
                      <span className={cn(!sublink.sectionId && 'text-slate-400')}>
                    ...
                    {sublink.sectionId && (
                      <button
                        onClick={() => onUpdate(subIndex, { ...sublink, sectionId: undefined })}
                        className="px-2 py-1.5 text-xs text-slate-500 hover:text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50"
                      >
                        Clear
                      </button>
                    )}
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-36 overflow-y-auto">
                      <button
                        ...
                        className={cn(
                          ...
                          !sublink.sectionId && 'bg-slate-100 font-medium'
                        )}
                      >
                      ...
                      {sections.map((section: PageSection) => (
                        <button
                          ...
                          className={cn(
                            'w-full px-2.5 py-1.5 text-sm text-left hover:bg-slate-50 transition-colors',
                            sublink.sectionId === section.id && 'bg-slate-100 font-medium'
                          )}
                        >
          ...
          {isFormType && (
            <div className="relative">
              ...
              <button
                ...
                className="w-full px-2.5 py-1.5 text-sm text-left bg-white border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 flex items-center justify-between"
              >
                <span className={cn(!sublink.formId && 'text-slate-400')}>
                  {sublink.formId ? forms?.find(f => f._id === sublink.formId)?.name || 'Select a form' : 'Select a form'}
                </span>
                <ChevronDown className="h-3.5 w-3.5 text-slate-400" />
              </button>
                    ...
                    forms.map((form) => (
                      <button
                        ...
                        className={cn(
                          ...
                          sublink.formId === form._id && 'bg-slate-100 font-medium'
                        )}
                      >
                        <span>{form.name}</span>
                  ...
                  <div className="flex gap-2">
                    {sublink.imageUrl && (
                      <div className="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 flex-shrink-0">
                        <img src={sublink.imageUrl} alt="" className="w-full h-full object-contain bg-slate-50" />
                      </div>
                    )}
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      {sublink.imageUrl ? 'Change' : 'Select'}
                    </button>
                    {sublink.imageUrl && (
                      <button
                        onClick={() => onUpdate(subIndex, { ...sublink, imageUrl: undefined })}
              ...
              {!sublink.useImage && (
                <>
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => onUpdate(subIndex, { ...sublink, hoverToImage: !sublink.hoverToImage })}
                      className={cn(
                        'relative w-9 h-4.5 rounded-full transition-all duration-200',
                        sublink.hoverToImage ? 'bg-slate-900' : 'bg-slate-300'
                      )}
                    >
                      <span className={cn(
                        'absolute top-0.5 w-3.5 h-3.5 bg-white rounded-full shadow transition-all duration-200',
                        sublink.hoverToImage ? 'right-0.5' : 'left-0.5'
                      )} />
                    </button>
                  </div>

                  {/* Hover Image Selection */}
                  {sublink.hoverToImage && (
                    <div>
                      <label className="block text-xs font-medium text-slate-700 mb-1">
                        Select Hover Image
                      </label>
                      <div className="flex gap-2">
                        {sublink.hoverImageUrl && (
                          <div className="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 flex-shrink-0">
                            <img src={sublink.hoverImageUrl} alt="" className="w-full h-full object-contain bg-slate-50" />
                          </div>
                        )}
                        <button
                          onClick={() => {
                        ...
                        >
                          ...
                          {sublink.hoverImageUrl ? 'Change' : 'Select'}
                        </button>
                        {sublink.hoverImageUrl && (
                          <button
                            onClick={() => onUpdate(subIndex, { ...sublink, hoverImageUrl: undefined })}
                            className="px-2 py-1.5 text-xs text-red-600 hover:text-red-700 border border-slate-200 rounded-lg hover:bg-red-50"
                          >
                            <X className="h-3 w-3" />
                          </button>
...
export function NavbarLinkEditor({ link, index, websiteId, userId, onUpdate, onRemove }: NavbarLinkEditorProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [showPageDropdown, setShowPageDropdown] = useState(false);
  const [showSectionDropdown, setShowSectionDropdown] = useState(false);
  const [showFormDropdown, setShowFormDropdown] = useState(false);
  ...
  // Toggle submenu
  const toggleSubmenu = () => {
    const newHasSubmenu = !link.hasSubmenu;
    onUpdate(index, {
      ...link,
      hasSubmenu: newHasSubmenu,
      submenu: newHasSubmenu ? (link.submenu || []) : undefined,
    });
  };

  // Add sublink
  const addSublink = () => {
    const newSublink: NavbarSubLink = {
      label: '',
      type: 'url',
      url: '',
    };
    onUpdate(index, {
      ...link,
      submenu: [...(link.submenu || []), newSublink],
    });
  };

  // Update sublink
  const updateSublink = (subIndex: number, updatedSublink: NavbarSubLink) => {
    const newSubmenu = [...(link.submenu || [])];
    newSubmenu[subIndex] = updatedSublink;
    onUpdate(index, { ...link, submenu: newSubmenu });
  };
  ...
  const removeSublink = (subIndex: number) => {
    ...
    onUpdate(index, {
      ...link,
      submenu: newSubmenu.length > 0 ? newSubmenu : undefined,
      hasSubmenu: newSubmenu.length > 0,
    });
  };
  ...
  return (
    <div className={cn(
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-4 py-3 bg-slate-50 hover:bg-slate-100 transition-colors rounded-t-xl"
      >
        <div className="flex items-center gap-3">
          {link.useImage && link.imageUrl ? (
            <img src={link.imageUrl} alt="" className="h-5 w-5 object-contain" />
      ...
      {isOpen && (
        <div className={cn(
          "p-4 space-y-4 bg-white border-t border-slate-200 rounded-b-xl",
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="flex-1 px-3 py-2 text-sm bg-white border border-slate-200 rounded-lg hover:border-slate-300 hover:bg-slate-50 transition-all flex items-center justify-center gap-2"
                    >
                      <ImageIcon className="h-4 w-4 text-slate-400" />
                      {link.imageUrl ? 'Change image' : 'Select from library'}
                    </button>
                    {link.imageUrl && (
                      <button
                        onClick={() => onUpdate(index, { ...link, imageUrl: undefined })}
                        className="px-2 py-2 text-xs text-red-600 hover:text-red-700 border border-slate-200 rounded-lg hover:bg-red-50 transition-colors"
                      >
                        Remove
                      </button>
                    )}
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          ...
                          {link.hoverImageUrl ? 'Change image' : 'Select hover image'}
                        </button>
                        {link.hoverImageUrl && (
                          <button
                      ...
                      <p className="text-[10px] text-slate-500 mt-1">
                        When hovered, text will rotate and show this image
                      </p>
                    </div>
                  )}
                </>
          ...
          <div className="border-t border-slate-200 pt-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-700">Dropdown Submenu</span>
              </div>
              <button
                onClick={toggleSubmenu}
                className={cn(
                  'relative w-10 h-5 rounded-full transition-all duration-200',
                  link.hasSubmenu ? 'bg-slate-900' : 'bg-slate-300'
                )}
              >
                <span className={cn(
                  'absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200',
                  link.hasSubmenu ? 'right-0.5' : 'left-0.5'
                )} />
              </button>
            </div>
            ...
            {link.hasSubmenu && (
              <div className="space-y-3">
                <p className="text-xs text-slate-500">
                  Add submenu items that appear when users hover over this link.
                </p>
                ...
                <div className="space-y-2">
                  {(link.submenu || []).map((sublink, subIndex) => (
                    <SublinkEditor
                      ...
                      websiteId={websiteId}
                      userId={userId}
                      onUpdate={updateSublink}
                      onRemove={removeSublink}
                      pages={pages || []}
                      forms={forms || []}
                    />
                  ))}
                ...
                <button
                  onClick={addSublink}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-sm font-medium text-slate-600 bg-slate-50 hover:bg-slate-100 border border-dashed border-slate-300 rounded-lg transition-all"
                >
