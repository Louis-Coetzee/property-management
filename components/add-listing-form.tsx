...
export default function AddListingForm({ className = '' }: AddListingFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const newListingId = await createListing(listingData);
      setListingId(newListingId as string);

      toast({
        title: '✅ Listing Created Successfully!',
        description: 'Your listing has been created. Redirecting to availability settings...',
        variant: 'default',
      });

      // Redirect to availability page
      setTimeout(() => {
        router.push(`/listings/${newListingId}/availability`);
      }, 1500);
    } catch (error) {
      toast({
        title: 'Error',
  ...
  return (
    <div className={`max-w-4xl mx-auto ${className}`}>
      <div className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Add Your Listing</h1>
        <p className="text-gray-600">
          List your accommodation and start earning income from travelers worldwide.
        </p>
      </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-between gap-3 p-4 bg-white rounded-lg border">
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              {currentStep < STEPS.length - 1 ? (
                ...
                >
                  {isSubmitting ? 'Creating...' : 'Create Listing & Set Availability'}
                </Button>
              )}
            </div>
          </div>
