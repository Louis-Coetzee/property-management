...
export default function EditTransactionPage() {
  ...
  const updateTransaction = useMutation(api.payments.update);
  const removeTransaction = useMutation(api.payments.remove);
  ...
  const [paymentScope, setPaymentScope] = useState<'full' | 'partial'>('partial');

  // Track original values to reconcile invoice amountPaid on update
  const [originalInvoiceId, setOriginalInvoiceId] = useState('');
  const [originalAmount, setOriginalAmount] = useState(0);
  const [originalStatus, setOriginalStatus] = useState<Status>('completed');
  ...
  const transaction = useQuery(
    ...
    user?.id && transactionId
      ? { userId: user.id as any, paymentId: transactionId as any }
      : 'skip'
  );
  ...
  useEffect(() => {
    if (transaction && !hasLoaded) {
      ...
      const txnType = (t.type as TransactionType) || (t.invoiceId ? 'income' : 'expense');
      setType(txnType);
      setAmount(t.amount || 0);
      setPaymentMethod((t.paymentMethod as PaymentMethod) || 'eft');
      const txnStatus = (t.status as Status) || 'completed';
      setStatus(txnStatus);
      setTransactionDate(t.transactionDate || new Date(t.createdAt).toISOString().split('T')[0]);
      ...
      setSource(t.source || '');
      setExpenseType(t.expenseType || 'operating');
      if (t.invoiceId) setIncomeSource('invoice');
      else if (txnType === 'income') setIncomeSource('other');
      if (t.supplierId) setExpenseSource('supplier');
      else if (txnType === 'expense') setExpenseSource('other');

      // Capture original values to reconcile invoice payment
      setOriginalInvoiceId(t.invoiceId || '');
      setOriginalAmount(t.amount || 0);
      setOriginalStatus(txnStatus);
  ...
  // When editing an existing transaction, the original amount is already counted
  // in invoice.amountPaid, so we add it back to get the true outstanding for THIS payment.
  const invoiceOutstanding = useMemo(() => {
    ...
    // If we're editing the same invoice this transaction was linked to, add back
    // the original payment amount so the user can pay the full (or any) remaining balance.
    if (
      originalInvoiceId &&
      originalInvoiceId === invoiceId &&
      originalStatus === 'completed'
    ) {
      return baseOutstanding + originalAmount;
    }
    return baseOutstanding;
  }, [selectedInvoice, originalInvoiceId, invoiceId, originalAmount, originalStatus]);

  // Auto-fill amount when invoice is freshly selected (not preloaded)
  useEffect(() => {
    if (incomeSource === 'invoice' && selectedInvoice && paymentScope === 'full') {
      setAmount(invoiceOutstanding);
    }
  }, [selectedInvoice, paymentScope, invoiceOutstanding, incomeSource]);

  // Reset to 0 when switching to partial (only if it currently matches the full outstanding)
  useEffect(() => {
    if (
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 1) Reconcile invoice payments on the original invoice, if any.
      //    If the original was completed and linked to an invoice, we need to
      //    undo that payment first, then re-apply the new amount (if applicable).
      if (
        originalInvoiceId &&
        originalStatus === 'completed' &&
        originalAmount > 0
        ...
        await reversePayment({
          ...
          invoiceId: originalInvoiceId as any,
      ...
      const payload: any = {
        userId: user.id as any,
        paymentId: transactionId as any,
        paymentNumber: (transaction as any)?.paymentNumber || `TXN-${transactionId}`,
        type,
        amount,
        paymentMethod,
        status,
        ...
        invoiceId: type === 'income' && incomeSource === 'invoice' ? (invoiceId as any) : undefined,
        clientId: type === 'income' && incomeSource === 'invoice' && clientId ? (clientId as any) : undefined,
        clientName: type === 'income' && incomeSource === 'invoice' ? clientName : undefined,
        supplierId: type === 'expense' && expenseSource === 'supplier' ? (supplierId as any) : undefined,
        supplierName: type === 'expense' && expenseSource === 'supplier' ? supplierName : undefined,
        source:
          type === 'income'
            ? incomeSource === 'other'
              ? source
      ...
      // 3) Re-apply payment if the (new) transaction is completed + linked to an invoice
      ...
      if (
        ...
        applyResult = await applyPayment({
          userId: user.id as any,
      ...
      if (applyResult?.isFullyPaid) {
        toast.success('Transaction updated — invoice marked as fully paid!');
      } else if (applyResult) {
        toast.success(
          `Transaction updated. Invoice outstanding: ${formatCurrency(
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      // Reverse the invoice payment first so amountPaid stays in sync
      if (
        originalInvoiceId &&
        originalStatus === 'completed' &&
        originalAmount > 0
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center">
          <p className="text-slate-900 font-semibold">Transaction not found</p>
          <button
            onClick={() => router.push(`/${domain}/companies/${companyId}/crm/transactions`)}
    ...
    if (step === 2) {
      return (
        <div>
          ...
          <p className="text-sm sm:text-base text-slate-500 mt-1">
            {type === 'income'
              ? 'Linked invoice or other source.'
              : 'Selected supplier or other expense.'}
          </p>
            ...
            {type === 'income' ? (
              <>
                <SourceOption
                  ...
                  description="Linked to an invoice"
                />
                <SourceOption
                  ...
                  icon={ArrowUpCircle}
                  title="Other Income"
                  description="Not linked to an invoice"
                />
              </>
            ) : (
              <>
                <SourceOption
                  ...
                  description="Paid to a known supplier"
                />
                <SourceOption
                  ...
                  title="Other Expense"
                  description="Not linked to a supplier"
                />
              </>
            )}
          </div>
                ...
                <div>
                  ...
                  >
                    ...
                    {(invoices as any[] | undefined)?.map((inv) => {
                      // Effective outstanding accounts for the original payment we're editing
                      const baseDue = Math.max(
                        0,
                        (inv.total || 0) - (inv.amountPaid || 0)
                      );
                      const due =
                        inv._id === originalInvoiceId && originalStatus === 'completed'
                          ? baseDue + originalAmount
                          : baseDue;
            ...
            {type === 'income' && incomeSource === 'other' && (
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Bank interest, Refund received"
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent text-sm"
                />
              </div>
            )}
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <SummaryCard
              ...
              highlight
            />
            <SummaryCard label="Type" value={type === 'income' ? 'Income' : 'Expense'} />
            <SummaryCard
              label="Payment Method"
              value={paymentMethods.find((m) => m.value === paymentMethod)?.label || paymentMethod}
            />
            <SummaryCard label="Status" value={status.charAt(0).toUpperCase() + status.slice(1)} />
            <SummaryCard label="Transaction Date" value={transactionDate} />
            {type === 'expense' && (
              <SummaryCard
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32 sm:pb-12">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-slate-500 truncate">
                  {(transaction as any)?.paymentNumber} · Step {step} of 4
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 border border-red-200 text-red-600 text-xs sm:text-sm font-medium rounded-xl hover:bg-red-50 transition-all"
              >
      ...
      <DeleteConfirmationModal
        ...
        itemName={(transaction as any)?.paymentNumber || 'Transaction'}
        itemSubtitle={
