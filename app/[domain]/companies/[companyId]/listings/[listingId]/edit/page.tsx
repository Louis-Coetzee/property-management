...
export default function EditListingPage() {
  ...
  useEffect(() => {
    if (listing && !initialized) {
      ...
      setMaxGuests(listing.maxGuests || 2);
      setPricePerNight(listing.pricePerNight || 0);
      setCleaningFee(listing.cleaningFee?.toString() || '');
      setSecurityDeposit(listing.securityDeposit?.toString() || '');
      setAmenities((listing.amenities || []).join(', '));
      setImages(listing.images || []);
      setFeaturedImage(listing.featuredImage || null);
      setCoverImageIndex(0);
      setLocationProvince(listing.location?.province || '');
      setLocationCity(listing.location?.city || '');
      setLocationAddress(listing.location?.address || '');
      setAvailableFrom(listing.availableFrom || new Date().toISOString().split('T')[0]);
      setAvailableTo(listing.availableTo || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
      setMinimumStay(listing.minimumStay || 1);
      setMaximumStay(listing.maximumStay?.toString() || '');
      setContactEmail(listing.contactEmail || '');
      setContactPhone(listing.contactPhone || '');
      setHouseRules(listing.houseRules || '');
      setCheckInTime(listing.checkInTime || '14:00');
      ...
      <div className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 py-3 text-sm overflow-x-auto">
            ...
            <ChevronRight className="h-4 w-4 text-slate-400 flex-shrink-0" />
            <span className="text-slate-900 font-medium whitespace-nowrap">Edit Listing</span>
          </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-slate-600 hidden sm:block truncate">
                    {listing.title}
                  </p>
                </div>
              </div>
